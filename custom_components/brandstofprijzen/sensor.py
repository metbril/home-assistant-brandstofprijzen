...
PLATFORM_SCHEMA = PLATFORM_SCHEMA.extend({
    ...
    vol.Optional(CONF_ICON, default=DEFAULT_ICON):
        cv.string,
    vol.Optional(CONF_UNIT_OF_MEASUREMENT, default=DEFAULT_UNIT_OF_MEASUREMENT):
        cv.string,
...
def setup_platform(hass, config, add_devices, discovery_info=None):
    ...
    for variable in config[CONF_MONITORED_VARIABLES]:
        variables.append(
            BrandstofprijzenSensor(variable,rest,config)
        )
...
class BrandstofprijzenSensor(Entity):
    ...
    def __init__(self, sensor_type, rest, config):
        ...
        self._name = config[CONF_PREFIX].rstrip() + " " + \
...
class BrandstofprijzenData(object):
    ...
    def update(self):
        ...
        try:
            ...
            data = []
            # Euro95
            data.append(soup_info[1].select('div:nth-of-type(3)')[0].get_text().split()[1].replace(',','.'))
            # Diesel
            data.append(soup_info[2].select('div:nth-of-type(3)')[0].get_text().split()[1].replace(',','.'))
            # LPG
            data.append(soup_info[3].select('div:nth-of-type(3)')[0].get_text().split()[1].replace(',','.'))
